...
{
    class TestPlayer: Core.GameObject
    {
        int side = 0;

        public TestPlayer() : this(new Core.Transform())
        {
        }
        public TestPlayer(Core.Transform _transform)
        {
            Load(_transform);
        }
        public TestPlayer(Vector2 position, Vector2 scale, float rotation = 0f) : this(new Core.Transform(position, scale, rotation))
        {
        }
        public TestPlayer(Vector2 position) : this(new Core.Transform(position, Vector2.One))
        {
        }
